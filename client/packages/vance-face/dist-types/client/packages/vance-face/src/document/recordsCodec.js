...
function parseRecordsMarkdown(body) {
    ...
    const items = firstBodyLine.startsWith('|')
    ...
    if (schema.length === 0) {
        throw new RecordsCodecError('Missing or empty schema — `kind: records` requires either a front-matter `schema:` line or a Markdown-table header row.');
    }
    return { kind, schema, items, extra };
}
function parseBulletBody(lines, start, schema) {
    const items = [];
 ...
 *  the records. */
function parseTableBody(lines, start, schemaIn, onHeader) {
    ...
    let i = start + 1;
    if (i < lines.length && isAlignmentDivider(lines[i]))
        i++;
    const items = [];
 ...
 *  is tolerated. */
function isAlignmentDivider(line) {
