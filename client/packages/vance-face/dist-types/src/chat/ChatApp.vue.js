...
import { WebSocketRequestError, getUsername, setActiveSessionId, } from '@vance/shared';
import { bindSession, ensureConnected as wsEnsureConnected, markBound, unbindNow, useWsConnection, } from '@/ws/wsConnectionStore';
...
// on top of it.
const { socket, status: wsStatus, activeSessionId, } = useWsConnection();
/**
 ...
 */
const socketDown = computed(() => wsStatus.value === 'reconnecting' || wsStatus.value === 'down');
...
watch(socket, (next) => {
    ...
    if (next) {
        switchToUnsubscribe = next.on('switch-to', (data) => { void onSwitchTo(data); });
        progressUnsubscribe = next.on('process-progress', recordProgress);
    }
}, { immediate: true });
/**
...
async function leaveLive() {
    ...
    if (!ok)
        return; // mode flipped to 'failed' inside openSocketForPicker
    mode.value = 'picker';
}
