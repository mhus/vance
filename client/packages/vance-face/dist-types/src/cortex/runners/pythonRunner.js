...
export const pythonRunner = {
    ...
    async execute({ doc, projectId, args }) {
        ...
        function startPolling() {
            ...
            const tick = async () => {
                if (detached || !executionId)
                    return;
                try {
                    const snap = await brainFetch('GET', `python/executions/${encodeURIComponent(executionId)}`
        ...
        let resp;
        try {
            resp = await brainFetch('POST', `python/execute?projectId=${encodeURIComponent(projectId)}`, {
                body: {
                    scriptId: doc.id,
                    args: toShellArgs(),
                    sourceName: doc.path,
            ...
            return {
                ...
                async cancel() {
                    if (!executionId)
                        return;
                    try {
                        await brainFetch('POST', `python/executions/${encodeURIComponent(executionId)}/cancel`
                            + `?projectId=${encodeURIComponent(projectId)}`);
                    }
                    catch (e) {
