...
function isTexDocument(doc) {
    ...
    if (TEX_EXTS.some((ext) => p.endsWith(ext)))
        return true;
    return COMPOSE_NAMES.some((name) => p.endsWith(name));
}
function isComposeFile(doc) {
    const p = doc.path.toLowerCase();
    return COMPOSE_NAMES.some((name) => p.endsWith(name));
}
/**
 ...
 */
function findSiblingCompose(doc, files) {
...
export const texRunner = {
    ...
    async execute({ doc, projectId }) {
        ...
        if (isComposeFile(doc)) {
            ...
            logLines.value.push(`[tex] Using compose file: ${composePath}`);
        }
        state.value = 'running';
        try {
            const resp = await brainFetch('POST', 'tex/compile', {
                body: {
                    composePath,
                    projectId,
                },
...
//# sourceMappingURL=texRunner.js.map