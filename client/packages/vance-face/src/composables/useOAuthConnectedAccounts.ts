...
export function useOAuthConnectedAccounts(): {
  ...
  const providers = ref<OAuthProviderListEntry[]>([]);
  ...
  async function reload(): Promise<void> {
    ...
    error.value = null;
    try {
      providers.value = await brainFetch<OAuthProviderListEntry[]>(
        'GET',
        'oauth/providers',
      );
    } catch (e) {
  ...
  function connect(providerId: string, returnTo?: string): void {
    ...
    const path = `${brainBaseUrl()}/brain/${encodeURIComponent(tenant)}/oauth/${
  ...
  async function disconnect(providerId: string): Promise<void> {
    ...
    try {
      await brainFetch<void>(
        'DELETE',
        `oauth/${encodeURIComponent(providerId)}/connection`,
      );
      await reload();
    } catch (e) {
