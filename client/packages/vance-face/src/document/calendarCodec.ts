...
export function parseCalendar(body: string, mimeType: string): CalendarDocument {
...
export function serializeCalendar(doc: CalendarDocument, mimeType: string): string {
  if (isJson(mimeType)) return serializeCalendarJson(doc);
  if (isYaml(mimeType)) return serializeCalendarYaml(doc);
  throw new CalendarCodecError(`Unsupported mime type for calendar: ${mimeType}`);
}

export function isCalendarMime(mimeType: string | null | undefined): boolean {
...
function parseCalendarJson(body: string): CalendarDocument {
...
function serializeCalendarJson(doc: CalendarDocument): string {
  return JSON.stringify(
    wrapJsonMeta(doc.kind || 'calendar', buildBody(doc)),
    null, 2,
  ) + '\n';
}
...
function parseCalendarYaml(body: string): CalendarDocument {
  if (body.trim() === '') return emptyCalendar();
...
function serializeCalendarYaml(doc: CalendarDocument): string {
  return dumpYamlBody(doc.kind || 'calendar', buildBody(doc));
}

// ── Promotion ────────────────────────────────────────────────────────

function promoteToDocument(obj: Record<string, unknown>): CalendarDocument {
...
function promoteEvents(raw: unknown): CalendarEvent[] {
...
function isKnownEventKey(k: string): boolean {
...
function buildBody(doc: CalendarDocument): Record<string, unknown> {
...
function eventToObject(ev: CalendarEvent): Record<string, unknown> {
  const o: Record<string, unknown> = {};
  o.id = ev.id;
  o.title = ev.title;
  o.start = ev.start;
  if (ev.end !== undefined) o.end = ev.end;
  if (ev.allDay) o.allDay = true;
  if (ev.location !== undefined) o.location = ev.location;
  if (ev.attendees.length > 0) o.attendees = [...ev.attendees];
  if (ev.recurrence !== undefined) o.recurrence = ev.recurrence;
  if (ev.color !== undefined) o.color = ev.color;
  if (ev.tags.length > 0) o.tags = [...ev.tags];
  if (ev.notes !== undefined) o.notes = ev.notes;
