 ...
 */
export function mergeYamlMultiDoc(body: string): Record<string, unknown> {
  const docs = yaml.loadAll(body, undefined, { schema: yaml.JSON_SCHEMA }) as unknown[];
  ...
  if (docs.length >= 2 && isObject(docs[0]) && typeof (docs[0] as Record<string, unknown>).kind === 'string') {
    const header = docs[0] as Record<string, unknown>;
    const bodyDoc = docs[1];
    const merged: Record<string, unknown> = { ...header };
    if (isObject(bodyDoc)) {
 ...
 */
export function dumpYamlMultiDoc(
  kind: string,
  body: Record<string, unknown>,
  headerExtra?: Record<string, unknown>,
): string {
  const header: Record<string, unknown> = { kind };
  if (headerExtra) {
    for (const [k, v] of Object.entries(headerExtra)) {
      if (isScalar(v) && k !== 'kind') header[k] = v;
    }
  }
