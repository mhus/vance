...
function parseBulletBody(lines: string[]): TreeItem[] {
  const root: TreeItem[] = [];
 ...
 *  on depth-0 lines. */
function parseIndentedBody(lines: string[]): TreeItem[] {
  const root: TreeItem[] = [];
  interface OpenLevel { depth: number; list: TreeItem[]; }
  const openLevels: OpenLevel[] = [{ depth: -1, list: root }];
  let lastDepth = -1;
  ...
  for (const raw of lines) {
    ...
    const parent = openLevels[openLevels.length - 1].list;
    const item: TreeItem = { text, children: [], extra: {} };
    parent.push(item);
    openLevels.push({ depth, list: item.children });
    lastDepth = depth;
  }
...
function stripMermaidRoot(text: string): string {
