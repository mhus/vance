...
export function treeToMarkmapMarkdown(doc: TreeDocument): string {
  const opts = readMindmapOptions(doc);
  const lines: string[] = [];
  if (hasFrontMatter(opts)) {
    lines.push('---');
    lines.push('markmap:');
    if (opts.colorFreezeLevel !== undefined) lines.push(`  colorFreezeLevel: ${opts.colorFreezeLevel}`);
    if (opts.maxWidth !== undefined) lines.push(`  maxWidth: ${opts.maxWidth}`);
    if (opts.initialExpandLevel !== undefined) lines.push(`  initialExpandLevel: ${opts.initialExpandLevel}`);
    lines.push('---');
    lines.push('');
  }
...
function readMindmapOptions(doc: TreeDocument): {
  initialExpandLevel?: number;
  colorFreezeLevel?: number;
  maxWidth?: number;
} {
  const block = doc.extra?.mindmap as Record<string, unknown> | undefined;
  const out: { initialExpandLevel?: number; colorFreezeLevel?: number; maxWidth?: number } = {};
  if (block && typeof block === 'object') {
    const lvl = block.initialExpandLevel;
    if (typeof lvl === 'number') out.initialExpandLevel = lvl;
    const freeze = block.colorFreezeLevel;
    if (typeof freeze === 'number') out.colorFreezeLevel = freeze;
    const mw = block.maxWidth;
    if (typeof mw === 'number') out.maxWidth = mw;
  }
  return out;
}

function hasFrontMatter(opts: ReturnType<typeof readMindmapOptions>): boolean {
  return opts.initialExpandLevel !== undefined
    || opts.colorFreezeLevel !== undefined
    || opts.maxWidth !== undefined;
}

function walk(item: TreeItem, depth: number, lines: string[]): void {
...
function decorate(text: string, icon: string | undefined, link: string | undefined): string {
  let body: string;
  if (link) {
...
function stringField(item: TreeItem, key: string): string | undefined {
