...
/**
 * End-to-end test for the Eddie hub mechanics:
 *
 * <ol>
 ...
 *       chat-process runs the {@code eddie} engine (not Arthur), driven by
 *       {@code SessionChatBootstrapper}'s SYSTEM-project branch.</li>
 *   <li>{@code EddieEngine.start} writes the Eddie-flavoured greeting and
 ...
 * SessionChatBootstrapper hub-branch, EddieEngine + bundledConfig).
 *
...
class EddieHubTest {
    ...
    private static final String EDDIE_ENGINE_NAME = "eddie";
    ...
    @Test
    @Order(1)
    void hubProjectExistsAfterConnect_andSessionCreateUsesEddieEngine()
            throws Exception {
        ...
        // detects kind=SYSTEM and starts the 'eddie' engine. Wait until
        // the hub-process row appears in Mongo.
        boolean eddieProcessAppeared = pollUntil(Duration.ofSeconds(15), () -> {
            Document p = findOne("think_processes",
                    Filters.and(
                            Filters.eq("tenantId", TENANT),
                            Filters.eq("thinkEngine", EDDIE_ENGINE_NAME)));
            return p != null;
        });
        assertThat(eddieProcessAppeared)
                .as("a think-process with engine=eddie should appear within 15s")
                .isTrue();

        Document eddieProcess = findOne("think_processes",
                Filters.and(
                        Filters.eq("tenantId", TENANT),
                        Filters.eq("thinkEngine", EDDIE_ENGINE_NAME)));
        assertThat(eddieProcess).isNotNull();
        assertThat(eddieProcess.getString("name"))
                .as("the bootstrapper names the chat-process 'chat'")
                .isEqualTo("chat");
        // Bundled-config path means recipeName stays null (no recipe used).
        assertThat(eddieProcess.getString("recipeName"))
                .as("eddie bypasses recipes — recipeName must be null")
                .isNull();
        // Eddie's bundled config writes its own promptOverride from the
        // classpath resource. Length isn't pinned here — we just assert
        // it's substantial and contains the persona keyword.
        String prompt = eddieProcess.getString("promptOverride");
        assertThat(prompt)
                .as("eddie promptOverride should come from eddie-prompt.md")
                .isNotNull()
                .contains("Eddie");

        String eddieProcessId = eddieProcess.getObjectId("_id").toHexString();

        // ─── 4. The greeting Eddie.start() wrote should be in chat_messages.
        boolean greetingArrived = pollUntil(Duration.ofSeconds(10), () -> {
            Document msg = findOne("chat_messages",
                    Filters.eq("thinkProcessId", eddieProcessId));
            return msg != null;
        });
        assertThat(greetingArrived)
                .as("EddieEngine.start() should append a greeting within 10s")
                .isTrue();
        ...
        assertThat(greeting.getString("content"))
                .as("greeting content should match EddieEngine.GREETING")
                .startsWith("Hi, I'm Eddie");
    }

    /**
     * End-to-end chat round-trip: with the hub session bound (from the
     * previous test method), send a real chat message via
     * {@code POST /debug/chat} and assert that EddieEngine produced an
    ...
    void chatPathProducesAssistantReply() throws Exception {
        Document eddieProcess = findOne("think_processes",
                Filters.and(
                        Filters.eq("tenantId", TENANT),
                        Filters.eq("thinkEngine", EDDIE_ENGINE_NAME)));
        assertThat(eddieProcess)
                .as("test 1 should have left an eddie chat-process behind")
                .isNotNull();
        String eddieProcessId = eddieProcess.getObjectId("_id").toHexString();

        long messagesBefore = mongo.getCollection("chat_messages")
                .countDocuments(Filters.eq("thinkProcessId", eddieProcessId));
        ...
        assertThat(replyArrived)
                .as("EddieEngine should have produced an assistant reply within 60s")
                .isTrue();
    ...
    /**
     * Tool-call round-trip: Eddie gets a deliberately unambiguous "leg
     ...
     * = the Eddie hub-process all materialise in Mongo.
     *
    ...
    void chatTriggersProjectCreate() throws Exception {
        ...
        assertThat(projectAppeared)
                .as("Eddie should have called project_create within 90s. "
        ...
        assertThat(chatProcess.getString("thinkEngine"))
                .as("regular project's chat-engine must be Arthur, not Eddie")
                .isEqualTo("arthur");

        // Cross-project parent — the whole point of phase 3.6.
        assertThat(chatProcess.getString("parentProcessId"))
                .as("Arthur's parentProcessId should point back to the Eddie hub-process")
                .isEqualTo(eddieProcessId);
    }
