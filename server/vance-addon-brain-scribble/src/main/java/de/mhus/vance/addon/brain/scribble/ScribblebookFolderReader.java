...
/**
 * Scans a scribblebook folder for {@code kind: scribble} sheets plus the
 * {@code _app.yaml} manifest. System-managed files (underscore-prefixed)
 * are excluded — they are outputs, not source sheets.
 */
@Component
public class ScribblebookFolderReader {

    public static final String APP_MANIFEST = "_app.yaml";

    private final DocumentService documentService;

    public ScribblebookFolderReader(DocumentService documentService) {
    ...
    public Scan scan(String tenantId, String projectId, String folder) {
        ...
        if (manifest.isEmpty()) {
            throw new ToolException("No scribblebook manifest at '" + manifestPath + "'.");
        }
        ApplicationDocument config = parseManifest(manifest.get());
        String landingPage = readLandingPage(config);

        String prefix = normalized + "/";
        List<DocumentDocument> all = documentService.listByKind(tenantId, projectId, ScribbleService.KIND);
    ...
    private ApplicationDocument parseManifest(DocumentDocument manifest) {
        ...
        if (!ApplicationCodec.supports(mime)) {
            throw new ToolException("Scribblebook manifest '" + manifest.getPath() + "' has mime '" + mime
        ...
        try (InputStream in = documentService.loadContent(manifest)) {
            ...
            throw new ToolException(
                    "Could not parse scribblebook manifest '" + manifest.getPath() + "': " + e.getMessage(), e);
        }
    }

    private static @Nullable String readLandingPage(ApplicationDocument config) {
        Object block = config.config().get(ScribblebookApplication.APP_NAME);
