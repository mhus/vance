...
class FenceExtractorTest {
    ...
    void extract_singleFormFence_parsesTypeAndAttrs() {
        String body = """
                ...
                ```vance-form
                data: vance:/data/x.records.json?kind=records
        ...
        assertThat(b.str("data")).isEqualTo("vance:/data/x.records.json?kind=records");
    ...
    void extract_recursesIntoColumnsContainer() {
        ...
        String body = "````vance-columns\n"
                + "```vance-form\n"
                + "data: vance:a.records.json?kind=records\n"
                + "```\n"
                + "````\n";
        List<FenceBlock> blocks = extractor.extract("p.md", body);
        ...
        assertThat(blocks.get(0).str("data")).isEqualTo("vance:a.records.json?kind=records");
    }
