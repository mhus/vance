 ...
 */
@Component
public class WorkspaceIndexRenderer {
    ...
    public String render(WorkspaceFolderReader.Scan scan, String workspaceTitle) {
        ...
        sb.append("---\n");
        sb.append("# ").append(workspaceTitle).append("\n\n");
        ...
        if (scan.pages().isEmpty()) {
            sb.append("Keine Pages in diesem Workspace.\n");
            return sb.toString();
        }
        ...
        // alphabetically.
        Map<String, List<WorkspacePage>> bySection = new LinkedHashMap<>();
        for (WorkspacePage p : scan.pages()) {
            bySection.computeIfAbsent(p.section(), k -> new java.util.ArrayList<>()).add(p);
        }
        List<WorkspacePage> topLevel = bySection.remove("");
    ...
    private void renderFlat(List<WorkspacePage> pages, String folder, StringBuilder sb,
                            boolean showDescriptions) {
        sb.append("## Pages\n\n");
        renderList(pages, folder, sb, showDescriptions);
    }

    private void renderList(List<WorkspacePage> pages, String folder, StringBuilder sb,
                            boolean showDescriptions) {
        for (WorkspacePage p : pages) {
