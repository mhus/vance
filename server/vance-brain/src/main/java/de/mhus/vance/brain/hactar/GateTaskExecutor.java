 ...
 * to answer, links it to the calling {@code hactar_tasks} row, and
 * returns {@link Optional#empty()} — completion arrives asynchronously
 * via {@link HactarInboxCompletionListener} when the user replies.
 *
 * <h3>YAML</h3>
 * <pre>
 * review:
...
@Slf4j
public class GateTaskExecutor implements HactarTypeExecutor {

    /** Payload kind that {@link HactarInboxCompletionListener} keys on. */
    ...
    public HactarTaskType type() {
        return HactarTaskType.GATE_TASK;
    }
    ...
    public Optional<TaskOutcome> execute(HactarTaskContext context) {
        ...
        try {
            ...
            log.warn("Hactar gate_task '{}' inbox create failed: {}",
        ...
        log.info("Hactar gate_task '{}' inbox item created id='{}' assignedTo='{}'",
    ...
    private void scheduleTimeoutTimer(HactarTaskContext context, HactarStateSpec state) {
        ...
        try {
            ...
            log.debug("Hactar gate_task '{}' timeout timer scheduled fireAt={}",
            ...
            log.warn("Hactar gate_task '{}' timeout timer insert failed: {} — gate continues without timeout",
                    state.name(), ex.getMessage());
        }
    }
