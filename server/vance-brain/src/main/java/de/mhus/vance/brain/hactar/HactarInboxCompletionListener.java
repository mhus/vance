 ...
 * for the waiting Hactar {@code gate_task} (plan §4.4, §6.4).
 *
...
@Slf4j
public class HactarInboxCompletionListener {
    ...
    public void onAnswered(InboxItemAnsweredEvent event) {
        ...
        Optional<HactarTaskDocument> taskOpt = taskService.findByInboxItemId(item.getId());
        if (taskOpt.isEmpty()) {
            log.warn("Hactar inbox listener: workflow.gate item '{}' has no linked task",
                    item.getId());
            return;
        }
        ...
        if (answer == null) {
            log.warn("Hactar inbox listener: item '{}' answered with null payload", item.getId());
    ...
    private void publish(HactarTaskDocument task, String outcome,
        ...
        eventBus.publish(new TaskCompletedEvent(
                ...
                task.getId(),
                task.getStateName(),
                HactarTaskType.GATE_TASK,
                outcome,
                output,
                errorMessage,
                0L,
                null));
        log.info("Hactar gate_task completion task='{}' outcome='{}'",
                task.getId(), outcome);
    }
}
