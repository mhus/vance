...
/**
 * Resolves the system session a Hactar workflow run uses for its
 * {@code agent_task} spawns. One session per workflow run — display
 * name {@code _hactar_<runId>}; reused for every {@code agent_task} of
...
@Slf4j
public class HactarSessionResolver {

    private static final String DISPLAY_NAME_PREFIX = "_hactar_";

    private final SessionService sessionService;

    /**
     * Get or create the {@code _hactar_<runId>} session for this run.
    ...
    private SessionDocument createFresh(
        ...
        SessionDocument created = sessionService.create(
                ...
                displayName,
                Profiles.SCHEDULER,
                /*clientVersion*/ "hactar",
                /*clientName*/ null,
                /*system*/ true);
        sessionService.markBootstrapped(created.getSessionId());
        log.info("Hactar system-session created project='{}' name='{}' sessionId='{}' runAs='{}'",
