 ...
 * scans {@code hactar_tasks} for {@link HactarTaskStatus#PENDING} rows
 * whose {@code nextAttemptAt} is due, atomically flips them to
 * {@link HactarTaskStatus#CLAIMED} via Mongo {@code findAndModify}, and
 * dispatches each claimed task to the matching {@link HactarProjectLane}
 * for execution (plan §6.2).
 *
 * <p>Optimistic version is enforced via the {@code @Version} field of
 * {@link HactarTaskDocument}: the CAS update predicates on the current
...
@Slf4j
public class HactarTaskClaimer {
    ...
    public void scan() {
        ...
        Query query = new Query(
                Criteria.where("status").is(HactarTaskStatus.PENDING)
                        .and("nextAttemptAt").lte(now))
                .with(org.springframework.data.domain.Sort.by("nextAttemptAt").ascending())
                .limit(CLAIM_BATCH);
        List<HactarTaskDocument> tasks = mongoTemplate.find(query, HactarTaskDocument.class);
        if (tasks.isEmpty()) return;

        String podId = podId();
        Instant claimedAt = Instant.now();
        for (HactarTaskDocument task : tasks) {
            Optional<HactarTaskDocument> claimed = taskService.claim(task.getId(), podId, claimedAt);
            if (claimed.isEmpty()) {
                log.debug("Hactar task {} claim lost to another pod", task.getId());
                continue;
            }
            HactarTaskDocument claimedTask = claimed.get();
