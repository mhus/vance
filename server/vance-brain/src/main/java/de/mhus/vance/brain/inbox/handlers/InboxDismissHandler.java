...
public class InboxDismissHandler implements WsHandler {
    ...
    @Override
    public String type() {
        return MessageType.INBOX_DISMISS;
    }
    ...
    public void handle(ConnectionContext ctx, WebSocketSession wsSession, WebSocketEnvelope envelope)
        ...
        try {
            ...
            sender.sendError(wsSession, envelope, 400,
                    "Invalid inbox-dismiss payload: " + e.getMessage());
            return;
        }
        ...
        Optional<MaximegalonDocument> updated = inboxService.dismiss(
                ctx.getTenantId(), request.getItemId(), ctx.getUserId());
        if (updated.isEmpty()) {
            sender.sendError(wsSession, envelope, 404, "Inbox item not found");
            return;
        }
        sender.sendReply(wsSession, envelope, MessageType.INBOX_DISMISS,
