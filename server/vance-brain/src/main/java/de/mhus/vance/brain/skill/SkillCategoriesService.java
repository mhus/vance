...
import de.mhus.vance.api.skills.SkillCategoryDto;
...
/**
 * Cascade-aware grouping order and labels for the user-facing skill picker.
 *
 * <p>Reads {@code _vance/config/skill_categories.yaml} through
 * {@link DocumentService#lookupCascade} — project → {@code _vance} →
 * bundled classpath default — the same tiering every skill resolves
 * through. The document is a <b>sort help, not a registry</b>, the same
 * contract as {@code RecipeCategoriesService} for the recipe picker
 * (recipes.md §6e, skills.md §4f):
 *
 * <ul>
 ...
 * must not stop a user from listing skills (fail-open, same philosophy
 * as report themes).</li>
 * </ul>
 *
 ...
 * and fall back to a humanised category id.
 */
@Service
...
public class SkillCategoriesService {
    ...
    public static final String CATEGORY_CONFIG_PATH = "_vance/config/skill_categories.yaml";

    private final DocumentService documentService;

    /**
     * Outcome of {@link #arrange} — the categories metadata and the skills
     * sorted for grouped rendering. A record instead of reusing the WS
     * response type: {@code ProcessSkillHandler} owns the response assembly,
     * this service owns ordering only.
     */
    public record Arrangement(List<SkillCategoryDto> categories, List<SkillSummaryDto> skills) {}

    /**
     * Orders the listed skills for grouped rendering and pairs them with
     * the category metadata.
     *
     * <p>Sort: documented categories first (document order), undocumented
     * categories next (alphabetical, so a tenant skill with a new
     ...
     */
    public Arrangement arrange(String tenantId, @Nullable String projectId, List<SkillSummaryDto> skills) {
        List<SkillCategoryDto> categories = categories(tenantId, projectId);
        Map<String, Integer> ranks = groupRanks(categories, skills);
        List<SkillSummaryDto> sorted = new ArrayList<>(skills);
        sorted.sort(Comparator.comparingInt((SkillSummaryDto dto) -> rankOf(dto, ranks))
                .thenComparing(SkillCategoriesService::displayName, String.CASE_INSENSITIVE_ORDER));
        return new Arrangement(categories, List.copyOf(sorted));
    }
     ...
     * no labels and clients fall back to humanising the skill's own
     * {@code category} value.
     */
    private List<SkillCategoryDto> categories(String tenantId, @Nullable String projectId) {
        Optional<LookupResult> hit = documentService.lookupCascade(
                tenantId, SkillLoader.effectiveProjectId(projectId), CATEGORY_CONFIG_PATH);
        if (hit.isEmpty()) {
            return List.of();
        }
        ...
        try {
            ...
            log.warn(
                    "SkillCategoriesService: ignoring malformed category document at '{}' (source {}): {}",
                    result.path(),
                    result.source(),
                    e.getMessage());
            return List.of();
     ...
     * }</pre>
     *
     * <p>Ids are normalised the same way the skill field is (trim,
     ...
     */
    private static List<SkillCategoryDto> parse(String content) {
        ...
        List<SkillCategoryDto> out = new ArrayList<>(list.size());
        for (int i = 0; i < list.size(); i++) {
            ...
            out.add(SkillCategoryDto.builder().id(normalizedId).title(title).build());
        }
     ...
     * then undocumented ones alphabetically. Skills without a category
     * are not in the map — {@link #rankOf} sends them to the end.
     */
    private static Map<String, Integer> groupRanks(List<SkillCategoryDto> categories, List<SkillSummaryDto> skills) {
        Map<String, Integer> ranks = new LinkedHashMap<>();
        int next = 0;
        for (SkillCategoryDto category : categories) {
            ranks.putIfAbsent(category.getId(), next++);
        }
        List<String> undocumented = skills.stream()
                .map(SkillSummaryDto::getCategory)
    ...
    private static int rankOf(SkillSummaryDto dto, Map<String, Integer> ranks) {
    ...
    /** Picker display label: skill title with the skill name as fallback. */
    private static String displayName(SkillSummaryDto dto) {
