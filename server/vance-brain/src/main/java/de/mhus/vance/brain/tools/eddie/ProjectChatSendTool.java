 ...
 * — Eddie's primary "tell the worker what to do" tool.
 *
 ...
 * stuck) lands later as a {@code ProcessEvent} in Eddie's pending
 * queue, routed by the standard {@code ParentNotificationListener}
 * because {@code project_create} set Eddie as the chat-process's
 * cross-project parent.
 *
 * <p>{@code fromUser} is set to {@code "eddie:<eddieProcessId>"} so
