...
/**
 * Reads one Vance hub-specific Markdown topic by name.
 *
...
@Slf4j
public class VanceDocsReadTool implements Tool {
    ...
    @Override
    public String name() {
        return "vance_docs_read";
    }

    @Override
    public String description() {
        return "Read a specific Vance hub documentation topic by name. "
                + "Use vance_docs_list first to see what's available. "
    ...
    public Map<String, Object> invoke(Map<String, Object> params, ToolInvocationContext ctx) {
        ...
        if (!r.exists()) {
            throw new ToolException("Vance doc not found: '" + name + "'. "
                    + "Use vance_docs_list to see what's available.");
        }
        try {
            ...
            throw new ToolException("Failed to read Vance doc '" + name + "': "
                    + e.getMessage(), e);
        }
    }
}
