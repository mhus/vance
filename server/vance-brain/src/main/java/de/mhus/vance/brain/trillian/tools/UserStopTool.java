...
public class UserStopTool implements Tool {
    ...
    public Map<String, Object> invoke(Map<String, Object> params, ToolInvocationContext ctx) {
        ...
        try {
            // Shared with the //trillian stop command — one implementation,
            // two entrances, so tool and command can never drift apart.
            return shape(peer.getName(), api.pausePeer(peer));
        } catch (RuntimeException e) {
            throw new ToolException("user_stop failed: " + e.getMessage(), e);
        }
    }
