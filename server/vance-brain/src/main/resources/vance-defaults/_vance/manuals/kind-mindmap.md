...
`doc_write(kind="mindmap", path="<…>", content=<raw>)`, the content
...
- **Wrapping the stored body in a ```` ```mindmap ```` fence.**
  That is the inline-chat form. When you save via `doc_write`,
...
Then call `doc_write(kind="mindmap", path="mindmaps/<name>.md",
