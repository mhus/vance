...
import de.mhus.vance.shared.memory.evaluation.AffectsExisting;
import de.mhus.vance.shared.memory.evaluation.CrossItemRelation;
import de.mhus.vance.shared.memory.evaluation.CrossItemRelationType;
import de.mhus.vance.shared.memory.evaluation.Decay;
import de.mhus.vance.shared.memory.evaluation.EvaluationOutput;
import de.mhus.vance.shared.memory.evaluation.Evidence;
import de.mhus.vance.shared.memory.evaluation.EvidenceRole;
import de.mhus.vance.shared.memory.evaluation.ExtractedItem;
import de.mhus.vance.shared.memory.evaluation.ItemCountExpectation;
import de.mhus.vance.shared.memory.evaluation.ItemType;
import de.mhus.vance.shared.memory.evaluation.LongTermMemoryAction;
import de.mhus.vance.shared.memory.evaluation.LongTermMemoryDecision;
import de.mhus.vance.shared.memory.evaluation.Scope;
...
class MemoryEvaluationSanitizerTest {

    private MemoryEvaluationProperties props;
    private MemoryEvaluationSanitizer sanitizer;

    @BeforeEach
    void setUp() {
        props = new MemoryEvaluationProperties();
        sanitizer = new MemoryEvaluationSanitizer(props);
    }
    ...
    void tokenize_splitsOnWordBoundariesAndLowercases() {
        assertThat(MemoryEvaluationSanitizer.tokenize("User prefers terse, answers!"))
                .containsExactly("user", "prefers", "terse", "answers");
    }

    @Test
    void contentSimilarity_identicalSentencesReturnsOne() {
        assertThat(MemoryEvaluationSanitizer.contentSimilarity(
    ...
    void labelOverlap_identicalSetsReturnOne() {
        assertThat(MemoryEvaluationSanitizer.labelOverlap(
    ...
    void contentSimilarity_identicalReturnsOne() {
        assertThat(MemoryEvaluationSanitizer.contentSimilarity("foo", "foo"))
                .isEqualTo(1.0);
    }
