...
/**
 * One append-only row in {@code hactar_journal}. Carries the typed
 * {@link de.mhus.vance.shared.hactar.journal.JournalRecord} body as a
 ...
 * the {@code HactarStateProjector} from a sequence of these entries
 * (plan §3.2).
 *
 ...
 */
@Document(collection = "hactar_journal")
@CompoundIndexes({
        @CompoundIndex(
        ...
        @CompoundIndex(
                ...
                unique = true,
                partialFilter = "{ 'type': 'de.mhus.vance.shared.hactar.journal.TaskResultRecord' }")
})
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class HactarJournalEntry {
    ...
    /** FQN of the concrete {@link de.mhus.vance.shared.hactar.journal.JournalRecord}. */
    private String type = "";

    /** Jackson-serialised record body. */
    private String data = "";

    /** Mongo {@code _id} of the {@link HactarTaskDocument} this entry belongs to (when applicable). */
