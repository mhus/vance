...
/**
 * Append-only journal owner for the Hactar workflow subsystem. Holds
 * datahoheit over the {@code hactar_journal} collection — all writes
 * and reads of journal entries go through this service.
 *
 * <p>Typed {@link JournalRecord} bodies are Jackson-serialised into the
 * generic {@link HactarJournalEntry} container. The {@code type} field
 ...
 * unique index defined on {@link HactarJournalEntry} so the second
...
@Slf4j
public class HactarJournalService {

    private final HactarJournalRepository repository;
    private final ObjectMapper objectMapper;

    // ──────────── append ────────────

    /** Append a record that isn't tied to a specific task (status, start, note, …). */
    public HactarJournalEntry append(
    ...
    public HactarJournalEntry append(
        ...
        HactarJournalEntry entry = buildEntry(tenantId, projectId, workflowRunId, taskId, record);
     ...
     */
    public Optional<HactarJournalEntry> appendIfAbsent(
            String tenantId,
            String projectId,
            String workflowRunId,
            String taskId,
    ...
    private HactarJournalEntry buildEntry(
            String tenantId,
            String projectId,
            String workflowRunId,
            @Nullable String taskId,
    ...
    public List<HactarJournalEntry> read(String workflowRunId) {
    ...
    public List<HactarJournalEntry> readByTaskId(String workflowRunId, String taskId) {
    ...
    public Optional<java.time.Instant> firstCreatedAt(String workflowRunId) {
        List<HactarJournalEntry> entries = repository.findByWorkflowRunIdOrderByCreatedAtAsc(workflowRunId);
    ...
    public <T extends JournalRecord> Optional<T> readLast(
        ...
        for (int i = entries.size() - 1; i >= 0; i--) {
            HactarJournalEntry entry = entries.get(i);
     ...
     * variable replay (typed {@link de.mhus.vance.shared.hactar.journal.VarRecord})
     * and audit trails.
     */
    ...
    public <T extends JournalRecord> T toRecord(HactarJournalEntry entry, Class<T> recordType) {
        if (!recordType.getName().equals(entry.getType())) {
     ...
     */
    public Optional<JournalRecord> toRecord(HactarJournalEntry entry) {
        try {
    ...
    private <T extends JournalRecord> T deserialize(HactarJournalEntry entry, Class<T> recordType) {
        try {
