...
/**
 * Published by {@link MaximegalonService} after a new item lands in
 ...
 * {@link MaximegalonAnsweredEvent} immediately — listeners that don't
 * want to ping the user about an item that's already resolved should
 * filter on {@code item.status} themselves.
 */
public record MaximegalonCreatedEvent(MaximegalonDocument item) {
}
