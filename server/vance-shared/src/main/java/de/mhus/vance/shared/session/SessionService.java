...
public class SessionService {
    ...
    /**
     * Creates a fresh session that copies {@code source}'s user-facing
     * metadata, lifecycle policy, and denormalised chat preview, but with
     * its own identity and a clean runtime state — the session-duplicate
     * path (see {@code SessionDuplicationService}).
     *
     * <p>Freshly minted / reset: {@code sessionId}, {@code status=INIT},
     * {@code boundConnectionId=null}, {@code chatProcessId=null} (relinked
     * by the caller after the chat-process copy), all suspend/archive
     * runtime timestamps, {@code pinned=false} and {@code userTouchedAt}
     * (a copy is not pinned or user-touched until the user acts on it),
     * {@code createdAt}/{@code lastActivityAt=now}. The chat process, its
     * history, and its memories are copied separately by the caller.
     *
     * @param source   the session to copy from
     * @param newTitle explicit title for the copy, or {@code null} to
     *                 carry over {@code source}'s title verbatim
     */
    public SessionDocument createCopy(SessionDocument source, @Nullable String newTitle) {
        Instant now = Instant.now();
        List<String> tags = source.getTags() == null
                ? new ArrayList<>()
        ...
        SessionDocument doc = SessionDocument.builder()
                ...
                .clientAgentDocPath(source.getClientAgentDocPath())
                .clientAgentDoc(source.getClientAgentDoc())
                .boundConnectionId(null)
                .status(SessionStatus.INIT)
                .onDisconnect(source.getOnDisconnect())
                .onIdle(source.getOnIdle())
                .onSuspend(source.getOnSuspend())
                .idleTimeoutMs(source.getIdleTimeoutMs())
                .suspendKeepDurationMs(source.getSuspendKeepDurationMs())
                .chatProcessId(null)
                .createdAt(now)
                .lastActivityAt(now)
                .firstUserMessage(source.getFirstUserMessage())
                .lastMessagePreview(source.getLastMessagePreview())
                .lastMessageRole(source.getLastMessageRole())
                .lastMessageAt(source.getLastMessageAt())
                .title(newTitle != null ? newTitle : source.getTitle())
                .titleAutoGenerated(false)
                .icon(source.getIcon())
                .color(source.getColor())
                .tags(tags)
                .pinned(false)
                .system(source.isSystem())
                .allowMultipleClients(source.isAllowMultipleClients())
                .build();
        SessionDocument saved = repository.save(doc);
        log.info("Duplicated session source='{}' → copy='{}' tenant='{}' user='{}' project='{}'",
                source.getSessionId(), saved.getSessionId(), saved.getTenantId(),
                saved.getUserId(), saved.getProjectId());
        return saved;
    }
