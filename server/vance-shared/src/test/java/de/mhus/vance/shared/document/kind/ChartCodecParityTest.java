...
/**
 * Java ↔ TS parity test for the {@code chart} kind-codec (Java side).
 *
 * <p>Reads the SHARED fixture corpus at
 * {@code <repo>/test-fixtures/kind-codecs/chart/*.json} — the very
 * same files consumed by {@code chartCodec.parity.test.ts}
 * (vance-face). For each fixture it runs
 * {@code ChartCodec.serialize(ChartCodec.parse(input, mime), "application/json")},
...
class ChartCodecParityTest {
    ...
    private void runFixture(JsonNode fixture) {
        ...
        ChartDocument doc = ChartCodec.parse(input, mime);
