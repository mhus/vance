...
/**
 * Java ↔ TS parity test for the {@code checklist} kind-codec (Java side).
 *
 * <p>Reads the SHARED fixture corpus at
 * {@code <repo>/test-fixtures/kind-codecs/checklist/*.json} — the very
 * same files consumed by {@code checklistCodec.parity.test.ts}
 * (vance-face). For each fixture it runs
 * {@code ChecklistCodec.serialize(ChecklistCodec.parse(input, mime), "application/json")},
 ...
 * {@code driftNote}). See the corpus README.
 */
class ChecklistCodecParityTest {

    private static final ObjectMapper MAPPER = new ObjectMapper();

    /** From module basedir {@code server/vance-shared} the repo root is {@code ../..}. */
    private static final File FIXTURE_DIR =
            new File("../../test-fixtures/kind-codecs/checklist");

    @TestFactory
    List<DynamicTest> checklistParityCorpus() throws IOException {
        if (!FIXTURE_DIR.isDirectory()) {
            throw new IllegalStateException(
                    "Shared fixture corpus not found at "
                            + FIXTURE_DIR.getCanonicalPath()
                            + " — expected repo-root test-fixtures/kind-codecs/checklist");
        }
    ...
    private void runFixture(JsonNode fixture) {
        ...
        // corpus without breaking the build.
        ...
        ChecklistDocument doc = ChecklistCodec.parse(input, mime);
        String json = ChecklistCodec.serialize(doc, "application/json");
