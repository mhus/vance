...
/**
 * Java ↔ TS parity test for the {@code diagram} kind-codec (Java side).
 *
 * <p>Reads the SHARED fixture corpus at
 * {@code <repo>/test-fixtures/kind-codecs/diagram/*.json} — the very
 * same files consumed by {@code diagramCodec.parity.test.ts}
 * (vance-face). For each fixture it runs
 * {@code DiagramCodec.serialize(DiagramCodec.parse(input, mime), "application/json")},
...
class DiagramCodecParityTest {
    ...
    private void runFixture(JsonNode fixture) {
        ...
        DiagramDocument doc = DiagramCodec.parse(input, mime);
        String json = DiagramCodec.serialize(doc, "application/json");
