...
/**
 * Java ↔ TS parity test for the {@code graph} kind-codec (Java side).
 *
 * <p>Reads the SHARED fixture corpus at
 * {@code <repo>/test-fixtures/kind-codecs/graph/*.json} — the very
 * same files consumed by {@code graphCodec.parity.test.ts}
 * (vance-face). For each fixture it runs
 * {@code GraphCodec.serialize(GraphCodec.parse(input, mime), "application/json")},
...
class GraphCodecParityTest {
    ...
    private void runFixture(JsonNode fixture) {
        ...
        GraphDocument doc = GraphCodec.parse(input, mime);
