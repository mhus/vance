...
/**
 * Java ↔ TS parity test for the {@code list} kind-codec (Java side).
 *
 * <p>Reads the SHARED fixture corpus at
 * {@code <repo>/test-fixtures/kind-codecs/list/*.json} — the very
 * same files consumed by {@code listItemsCodec.parity.test.ts}
 * (vance-face). For each fixture it runs
 * {@code ListCodec.serialize(ListCodec.parse(input, mime), "application/json")},
...
class ListCodecParityTest {
    ...
    private void runFixture(JsonNode fixture) {
        ...
        ListDocument doc = ListCodec.parse(input, mime);
        String json = ListCodec.serialize(doc, "application/json");
