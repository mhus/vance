...
/**
 * Java ↔ TS parity test for the {@code sheet} kind-codec (Java side).
 *
 * <p>Reads the SHARED fixture corpus at
 * {@code <repo>/test-fixtures/kind-codecs/sheet/*.json} — the very
 * same files consumed by {@code sheetCodec.parity.test.ts}
 * (vance-face). For each fixture it runs
 * {@code SheetCodec.serialize(SheetCodec.parse(input, mime), "application/json")},
...
class SheetCodecParityTest {
    ...
    private void runFixture(JsonNode fixture) {
        ...
        SheetDocument doc = SheetCodec.parse(input, mime);
