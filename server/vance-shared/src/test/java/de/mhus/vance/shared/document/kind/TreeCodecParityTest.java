...
/**
 * Java ↔ TS parity test for the {@code tree} kind-codec (Java side).
 *
 * <p>Reads the SHARED fixture corpus at
 * {@code <repo>/test-fixtures/kind-codecs/tree/*.json} — the very
 * same files consumed by {@code treeItemsCodec.parity.test.ts}
 * (vance-face). For each fixture it runs
 * {@code TreeCodec.serialize(TreeCodec.parse(input, mime), "application/json")},
...
class TreeCodecParityTest {
    ...
    private void runFixture(JsonNode fixture) {
        ...
        TreeDocument doc = TreeCodec.parse(input, mime);
        String json = TreeCodec.serialize(doc, "application/json");
