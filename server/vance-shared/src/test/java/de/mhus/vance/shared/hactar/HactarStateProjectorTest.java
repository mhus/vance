...
/**
 * Replay-logic tests for {@link HactarStateProjector}. Drives a real
 * {@link HactarJournalService} (real Jackson serialisation) over a
 * Mockito-backed repository so the projector sees the same byte stream
 * a production run would produce.
 */
class HactarStateProjectorTest {

    private final ObjectMapper objectMapper = JsonMapper.builder().build();
    private final HactarJournalRepository repo = mock(HactarJournalRepository.class);
    private final HactarJournalService journal;
    private final HactarStateProjector projector;

    private final List<HactarJournalEntry> entries = new ArrayList<>();
    private Instant tick = Instant.parse("2024-01-01T00:00:00Z");

    HactarStateProjectorTest() {
        when(repo.save(any(HactarJournalEntry.class))).thenAnswer(inv -> {
            HactarJournalEntry e = inv.getArgument(0);
        ...
        journal = new HactarJournalService(repo, objectMapper);
        projector = new HactarStateProjector(journal, objectMapper);
    }
    ...
    void project_running_workflow_returns_RUNNING_with_current_state() {
        ...
        HactarProcessDto dto = projector.project("r1").orElseThrow();
        ...
        assertThat(dto.getStatus()).isEqualTo(HactarRunStatus.RUNNING);
    ...
    void project_marks_terminal_DONE_with_terminatedAt_and_result() {
        ...
        append(ResultRecord.builder()
                ...
                .build());
        append(StatusRecord.builder().status(HactarRunStatus.DONE).build());

        HactarProcessDto dto = projector.project("r1").orElseThrow();

        assertThat(dto.getStatus()).isEqualTo(HactarRunStatus.DONE);
    ...
    void project_marks_terminal_FAILED_status() {
        ...
        append(StatusRecord.builder().status(HactarRunStatus.FAILED).reason("checks failed").build());

        HactarProcessDto dto = projector.project("r1").orElseThrow();

        assertThat(dto.getStatus()).isEqualTo(HactarRunStatus.FAILED);
    ...
    void projectStatus_returns_RUNNING_when_no_status_record_yet() {
        ...
        assertThat(projector.projectStatus("r1")).isEqualTo(HactarRunStatus.RUNNING);
    }

    @Test
    void projectStatus_returns_latest_status() {
        append(StatusRecord.builder().status(HactarRunStatus.RUNNING).build());
        append(StatusRecord.builder().status(HactarRunStatus.PAUSED).build());
        append(StatusRecord.builder().status(HactarRunStatus.DONE).build());

        assertThat(projector.projectStatus("r1")).isEqualTo(HactarRunStatus.DONE);
    }
